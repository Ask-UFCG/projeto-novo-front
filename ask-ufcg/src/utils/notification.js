...
const _gerarMensagemErro500 = (error, status) => {
  ...
  if (_existeMensagemRegraNegocioDoBanco(error.response.data)) {
    ...
    error.response.data.indexOf('Could not get JDBC Connection') !== -1
  ) {
    mensagem = 'Ocorreu um erro na comunicação com o banco de dados';
  } else if (
    error.response.data.indexOf(
      'Already exists a object with the same identifier'
    ) !== -1
  ) {
...
const _existeMensagemRegraNegocioDoBanco = (msg) => {
  ...
  if (typeof msg === 'string') {
    posicaoEncontrada = msg.search(/^.*?ORA-\d*:\s*([^,]*)/);
  }
...
const _capturarMensagemRegraNegocioDoBanco = (msg) => {
  ...
  if (
    ...
    msgORA =
      'Não é permitida a exclusão do registro, pois existem registros filhos';
  }
